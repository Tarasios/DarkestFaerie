/* =====================================================================
   Shared helpers for the Darkest Faerie tracker tools — the small utilities that
   used to be copy-pasted into js/df-tracker.js, js/df-bbs-tracker.js,
   js/df-melding.js and js/landing.js. Loaded (as a plain <script>) before
   any of those, it publishes a single global `DF`; each consumer pulls in
   what it needs, e.g.
   ...
   Game DATA still lives in the per-game js/df-*-tracker-data.js modules;
...
window.DF = (function () {
  ...
  function toast(text, icon) {
    let host = document.getElementById("df-toasts");
    if (!host) { host = el("div"); host.id = "df-toasts"; document.body.appendChild(host); }
    const node = el("div", "df-toast");
    node.innerHTML = `<span class="df-toast-ic">${icon || "✅"}</span><span>${esc(text)}</span>`;
