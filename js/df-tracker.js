/* =====================================================================
   Generic DF game completion tracker engine.

   Drives every game except Birth by Sleep (which has its own engine in
   js/df-bbs-tracker.js). One page = one game data module that defines the
   ...
   Depends on: js/df-common.js (DF.*), js/i18n.js (i18n) and a game data
   module (see js/df-com-tracker-data.js for the shape):
...
const { el, esc, fmtText, chip, bar, toast } = DF;

const game = TRACKER_GAME;
const translate = (key) => i18n.getMessage(key);
const format = (key, ...args) => i18n.format(key, ...args);

/* A coloured element pill (Fire/Water/Air/Earth/Light/Dark/…). The slug
   drives the colour via the .el-<slug> classes in css/df.css; the opposing
   element (extra damage / deflect target) is shown on hover. */
const ELEMENT_OPPOSITE = { fire: "Water", water: "Fire", air: "Earth", earth: "Air", light: "Dark", dark: "Light" };
function elementChip(value) {
  const slug = String(value).toLowerCase().replace(/[^a-z]+/g, "");
  const opp = ELEMENT_OPPOSITE[slug];
  const title = opp ? format('gt-element-tip', value, opp) : "";
  return `<span class="element el-${esc(slug)}"${title ? ` title="${esc(title)}" tabindex="0"` : ""}>${esc(value)}</span>`;
}
...
/* Per-group note shown under a group header: lang "gnote-<storeId>" (per
   ...
   through fmtText, so it may carry a link). E.g. DF1 Trinity unlocks, or the
...
function checklist(container, section, view, panelState) {
  ...
  view.items.forEach((item, index) => {
    ...
    if (panelState.mOnly && !item.m) return;
    ...
    const row = el("tr", ((done ? "donerow" : "") + (item.m ? " missable" : "")).trim() || null);
    if (checks) {
    ...
    cols.forEach(col => {
      ...
      let html;
      if (col.element && value) html = elementChip(value);
      else if (col.rarity && value) html = `<span class="rarity ${esc(value.toLowerCase())}">${esc(value)}</span>`;
      else if (col.name) html = `<span class="itemname">${fmtText(value)}</span>`
        + (item.m ? ` <span class="missbadge" title="${esc(translate('gt-missable-tip'))}">${translate('gt-missable-badge')}</span>` : "")
        + (auto ? ` <span class="srcbadge" title="${format('gt-auto-tip', auto)}">${translate('gt-auto-badge')}</span>` : "");
...
function buildPage() {
  ...
  gameTabs().forEach(tab => {
    ...
    const panelState = (PANEL[tab.id] && PANEL[tab.id].state) || { q: "", hide: false, mOnly: false };
    const hasTrophies = tab.sections.some(section => section.trophies);
    const hasMissable = tab.sections.some(section => {
      const lists = section.variants ? Object.values(section.variants) : [section.items || []];
      return lists.some(items => (items || []).some(item => item.m));
    });
    ...
    let missLabel = null;
    if (hasMissable) {
      missLabel = el("label", "hidelbl misslbl");
      const missToggle = el("input", "chk");
      missToggle.type = "checkbox"; missToggle.checked = panelState.mOnly;
      missToggle.addEventListener("change", () => { panelState.mOnly = missToggle.checked; render(); });
      missLabel.appendChild(missToggle);
      missLabel.appendChild(document.createTextNode(" " + translate('gt-missable-only')));
    }
    const countBadge = el("span", "countbadge");
    const progressBar = el("span", "dashbar");
    progressBar.appendChild(el("i"));
    toolbar.appendChild(filterInput); toolbar.appendChild(hideLabel); if (missLabel) toolbar.appendChild(missLabel);
    toolbar.appendChild(countBadge); toolbar.appendChild(progressBar);
    if (hasTrophies) {
...
function worldEntryTable(entries) {
  ...
  entries.forEach(entry => {
    const row = el("tr", ((entry.done ? "donerow" : "") + (entry.m ? " missable" : "")).trim() || null);
    ...
    if (entry.m) nameHtml += ` <span class="missbadge" title="${esc(translate('gt-missable-tip'))}">${translate('gt-missable-badge')}</span>`;
  ...
  cfg.worlds.forEach((world, worldIndex) => {
    ...
    sectionViews.forEach(sectionView => {
      ...
      sectionView.view.items.forEach((item, index) => {
        ...
        entries.push({ store: sectionView.store, index, done: !!sectionView.store[index] || !!auto, auto, name, m: item.m,
